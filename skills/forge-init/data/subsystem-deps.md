...
3. **Frontend shared assets rule:** If ANY of `templ`, `htmx`, `alpine`, or `tailwind` is selected, the project gets a shared `assets/` directory tree and a single `//go:embed assets/static` directive with `forge.WithStaticFiles("/static/", staticFS, "assets/static")`. These are NOT duplicated per frontend option — they are added once.
